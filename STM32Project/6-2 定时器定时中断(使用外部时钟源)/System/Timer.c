...
void Timer_Init(void)
{
	...
	// 1. 开启外部时钟
	// RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2,ENABLE); // 使用内部时钟，TIM2是APB1总线的外设
	TIM_ETRClockMode2Config(TIM2, TIM_ExtTRGPSC_OFF, TIM_ExtTRGPSC_OFF, 0X00); // 使用外部红外对射式传感器作为时钟源
	...
	TIM_TimeBaseInitStruct.TIM_Period = 10 - 1;		  // ARR自动重装寄存器的值，从0计到9
	TIM_TimeBaseInitStruct.TIM_Prescaler = 1 - 1;	  //	PSC预分频器的值，不需要分频
